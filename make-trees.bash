...
;  (compile-system "tsdb" :force t)
  ...
  #+sbcl           (sb-ext:quit)
LISP
    } | ${LOGONROOT}/bin/logon --binary -I base -locale ja_JP.UTF-8 >> $log
    else
 	echo "Couldn't find  treebank at $tb"
    fi
done

echo "Missing any Trees?"
rgrep "No tree reconstructed" $outdir/trees

echo "Missing any MRSes?"
rgrep "No MRS available" $outdir/trees
